...
void SpawnBulletCommand::Execute()
{
	...
	m_GoBullet->AddComponent<TransformComponent>()->SetPosition(GetGameActor()->GetLocalPosition().x + 12.f,
	...
	for(const auto& object : m_pOtherGameObjects)
	{
		if(!object->IsDeleted())
		{
			object->GetComponent<BoxTriggerComponent>()->AddOtherObject(m_GoBullet);
